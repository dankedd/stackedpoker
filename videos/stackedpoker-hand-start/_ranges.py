...
LABEL = {"raise": "Raise", "3bet": "3-Bet", "4bet": "4-Bet", "call": "Call",
...
class PureRange:
    """A complete_strategy chart: every hand carries exactly ONE pure action, so
    every cell is a solid block of that action's colour. Sparse by convention —
    a hand absent from the chart is a genuine fold (threebetResponseBaselines.ts).

    Source: THREEBET_RESPONSE_CHARTS['HJ_vs_CO_3bet_response']. That file states
    plainly that its hand-level cells are an ILLUSTRATIVE CONSTRUCTION calibrated
    to Modern Poker Theory's stated aggregate (4-bet 22.2 / call 14.5 / fold 63.3),
    not a solver-exact chart, and that every lesson step using it says so on
    screen. The film carries the same marker, for the same reason."""

    ORDER = ["4bet", "call", "fold"]          # actionStyles.ts ACTION_PRIORITY

    def __init__(self, hands, title, highlight=None, note=None):
        self.hands, self.title = hands, title
        self.highlight, self.note = highlight, note

    def cell(self, hand):
        return 1.0, COLOR[self.hands.get(hand, "fold")], COLOR["fold"]

    def legend(self):
        return [(COLOR[a], LABEL[a]) for a in self.ORDER]


HJ_RESPONSE = PureRange(
    json.load(io.open(os.path.join(_HERE, "_hjco.json"), encoding="utf-8")),
    "HJ RESPONSE vs CO 3-BET", highlight="TT", note="illustrative")
...
def grid_html(rng, idp, build):
    ...
    return (f'<div class="sp-range" id="{idp}">'
            ...
            f'<div class="sp-range-legend" id="{idp}-legend">{legend}</div>'
            # threebetResponseBaselines.ts states its hand-level cells are an
            # illustrative construction calibrated to the book's aggregate, and
            # that every step using it says so on screen. So does the film.
            + ('<div class="sp-range-note" id="' + idp + '-illus">Illustrative '
               '&mdash; calibrated to the book&rsquo;s stated 22.2 / 14.5 / 63.3</div>'
               if getattr(rng, "note", None) else "")
            + "</div>")
