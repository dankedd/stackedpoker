...
def mobile_table_html(card_html, show_cards=True, scen=None) -> str:
    """The real PreflopTable (MOBILE_LAYOUT) in `scen`'s settled state."""
    if scen is None:
        import _scen as scen
    p = ['<div class="mt-table">',
         ...
         f'<span class="mt-pot-l">Pot</span>'
         f'<span class="mt-pot-v">{scen.POT_BB:g} BB</span></div>']

    for c in scen.CHIPS:
        v = f'<span class="mt-verb">{c["verb"]}</span>' if c["verb"] else ""
        p.append(
            f'<div class="mt-chips" style="left:{c["x"]}%;top:{c["y"]}%"><div class="mt-pile">'
            f'<div class="mt-chip mt-chip-{c["tone"]}" style="left:0;top:{CHIP_SPREAD}px;opacity:0.65">'
            f'<div class="mt-chip-face"></div></div>'
            f'<div class="mt-chip mt-chip-{c["tone"]}" style="left:{CHIP_SPREAD}px;top:0">'
            f'<div class="mt-chip-face"></div></div></div>'
            f'<span class="mt-amt mt-amt-{c["tone"]}">{c["amt"]}</span>{v}</div>')

    if show_cards:
        p.append(f'<div class="mt-cards" style="top:{CARD_ZONE_Y}%">'
                 f'<div class="mt-glow"></div>'
                 f'{card_html("T", "h")}{card_html("T", "c")}</div>')

    for st in scen.SEATS:
        cls = "mt-seat" + (" mt-seat-folded" if st["folded"] else "")
        pre = '<span class="mt-hero-prefix">HERO &middot;</span>' if st["hero"] else ""
        pc = "mt-pos mt-pos-hero" if st["hero"] else "mt-pos"
        d = '<span class="mt-dealer">D</span>' if st["dealer"] else ""
        p.append(f'<div class="{cls}" style="left:{st["lx"]}%;top:{st["ly"]}%">{pre}'
                 f'<span class="{pc}">{st["pos"]}</span>{d}</div>')
        if st["mx"] is not None:
            p.append(f'<div class="mt-meta" style="left:{st["mx"]}%;top:{st["my"]:.2f}%">'
                     f'<span class="mt-stack">{st["stack"]}</span></div>')
    p.append("</div>")
    return "".join(p)


def mobile_status_html(scen=None) -> str:
    if scen is None:
        import _scen as scen
    return ('<div class="mt-status">'
            f'<p class="mt-status-1">{scen.STATUS_LINE}</p>'
            f'<p class="mt-status-2">{scen.CENTER_STATUS}</p></div>')


def mobile_options_html(resolved: bool, scen=None) -> str:
    if scen is None:
        import _scen as scen
    if resolved:
        css = ["mt-opt mt-opt-dim"] * len(scen.OPTIONS)
        css[scen.CORRECT_INDEX] = "mt-opt mt-opt-sel"
    else:
        css = ["mt-opt mt-opt-idle"] * len(scen.OPTIONS)
    return ('<div class="mt-opts">' + "".join(
        f'<div class="{c}"><span style="position:relative">{l}</span></div>'
        for c, l in zip(css, scen.OPTIONS)) + "</div>")
