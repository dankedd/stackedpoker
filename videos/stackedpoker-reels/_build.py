# -*- coding: utf-8 -*-
"""StackedPoker — high-density 9:16 performance film. 1080x1920, 22.0s."""
import io, os, sys
...
# ── display scales (built at MBUILD, always shown downscaled) ───────────────
CARD_HOOK  = 1.00
TABLE_S    = 0.395
CARD_DEC   = 0.40
OPTS_S     = 0.4835
...
# major entrances use the brief's own curve
...
# The grid at phone width, declared AFTER product_css so it wins the cascade.
PHONE_RANGE = f"""
...
{body}
  ...
  <script>
    (function () {{
      ...
      var EASE = "{EASE}";
      /* Every inherited state is written by GSAP in EVERY frame, so both sides
         of a hard cut come out of one identical code path. */
...
def range_mount(idp, rng, scale=RANGE_S, cy=RANGE_CY):
    return f"""
    <div class="mount-c {idp}-rangewrap" id="{idp}-rangewrap" data-scale="{scale:.6f}"
         style="left:{RANGE_CX}px; top:{cy}px">
      {grid_html(rng, idp + "-range", MBUILD)}
    </div>"""
