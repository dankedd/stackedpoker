 ...
 * representations). Most steps don't and are unaffected.
 */
 ...
 * Fires whenever the step carries a ladder and hasn't exhausted it.
 */
export function shouldInjectRemediation(step: LessonStep, quality: ActionQuality): boolean {
  ...
  return missCount - 1 < ladder.length
}

/** Pick the next rung of the ladder for the current miss count (1-indexed → array index). */
export function pickRemediationStep(step: LessonStep): LessonStep | null {
  const ladder = step.remediation_ladder
  if (!ladder || ladder.length === 0) return null
  const missCount = getConceptMisses(step.concept_ids?.[0])
  const rung = ladder[Math.max(0, Math.min(missCount - 1, ladder.length - 1))]
  return rung ?? null
}

/**
 * Single entry point for LessonPlayer: given the step just answered and its
 * result, return the remediation step to insert right after it on a miss,
 * or null if nothing should be injected.
 */
export function pickInjectedStep(step: LessonStep, result: StepResult): LessonStep | null {
  if (shouldInjectRemediation(step, result.quality)) {
    return pickRemediationStep(step)
  }
  return null
}
