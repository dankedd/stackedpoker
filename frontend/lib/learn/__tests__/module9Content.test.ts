...
// Locks the exact combo-removal numbers Lessons 9.3 ("Block the Value") and
// 9.4 ("Leave the Bluffs Alive") state in their copy — board Ah 9d 4c 2s 7h,
// Hero holding Ac (+ Td or 7c). If curriculum content ever edits these cards
// without updating this test, the mismatch is the signal something drifted.
describe('Module 9 content — combo math the lessons state must stay true', () => {
  ...
  // ── Lesson 9.5 (Same Strength, Different Cards): AA/76s/65s removal for
  ...
  // ── Lesson 9.6/9.7 (Nut Blocker / Not Always Good). The ace is the top
  ...
  // ── Lesson 9.9 (Read the Removal): each scenario gives Hero exactly ONE
  ...
  // ── Lesson 9.10 (Blocker Lab) reuses bl-s2 (KK/Kd) and bl-s5 (flush pyramid,
  ...
  const MODULE_9_LESSON_IDS = [
    'from-hands-to-combos', 'the-range-after-removal', 'block-the-value', 'leave-the-bluffs-alive',
