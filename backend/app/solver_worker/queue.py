...
def _config_fingerprint(config: SolveJobConfig) -> str:
    ...
    key_parts = [
        ...
        ",".join(f"{s:.2f}" for s in sorted(config.turn_bet_sizes)),
        ",".join(f"{s:.2f}" for s in sorted(config.turn_raise_sizes)),
        ",".join(f"{s:.2f}" for s in sorted(config.river_bet_sizes)),
        ",".join(f"{s:.2f}" for s in sorted(config.river_raise_sizes)),
