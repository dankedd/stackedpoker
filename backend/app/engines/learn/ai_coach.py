...
async def generate_coach_reply(
    ...
    # Onboarding self-assessment signal — the learner's own self-reported
    # experience level, kept separate from active_leaks (which comes from
    # real lesson mistakes) so the two signals don't get diluted together.
    if context.get("experience_level"):
        ctx_parts.append(f"Learner self-reported their poker experience as: {context['experience_level']}.")
